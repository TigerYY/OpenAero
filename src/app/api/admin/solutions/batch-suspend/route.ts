...
// POST /api/admin/solutions/batch-suspend - 批量临时下架方案
export async function POST(request: NextRequest) {
  try {
    ...
    const validatedData = batchSuspendSchema.parse(body);

    // 验证所有方案都存在且状态为 PUBLISHED
    ...
    if (invalidStatusSolutions.length > 0) {
      return createErrorResponse(
        `以下方案状态不正确，必须为"已发布": ${invalidStatusSolutions.map(s => s.title).join(', ')}`,
        400
      );
    }
    ...
    await logAuditAction(request, {
      ...
      action: 'SOLUTION_BATCH_SUSPENDED',
      resource: 'solution',
    ...
    return createSuccessResponse(
      ...
      `成功临时下架 ${results.success.length} 个方案${results.failures.length > 0 ? `，失败 ${results.failures.length} 个` : ''}`
    );
  } catch (error) {
    console.error('批量临时下架失败:', error);
    ...
    return createErrorResponse(
      error instanceof Error ? error : new Error('批量临时下架失败'),
      500
    );
  }
}
