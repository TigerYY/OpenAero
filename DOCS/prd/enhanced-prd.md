...
- [x] 订单创建和管理
- [x] 订单状态跟踪（PENDING, CONFIRMED, PROCESSING, SHIPPED, DELIVERED,
      CANCELLED, REFUNDED）
- [x] 订单物流跟踪
- [x] 订单退款流程
- [x] 订单历史记录
...
**描述**: 完整的产品商城系统，包括产品搜索、筛选、评价、推荐、详情展示和产品数据管理。

**验收标准**:

- [x] 产品搜索功能（多字段搜索：名称、描述、品牌、SKU、型号）
- [x] 高级筛选功能（品牌、评分、库存、价格范围、分类）
- [x] 产品排序功能（价格、评分、销量、评价数、名称、创建时间）
- [x] 产品列表页面（响应式布局、分页、商品卡片展示）
- [x] 产品详情页面（完整的产品信息展示）
- [x] 产品配置清单（部件分类、部件名称、规格说明、单价、总价）
- [x] 产品报价说明（价格说明、产品特点、技术支持、售后服务、交付周期、付款方式、报价有效期）
- [x] 产品图片管理（SVG占位图生成、图片路径管理）
- [x] 产品数据导入（从HTML文档批量导入产品数据）
...
- Product数据模型（包含ProductComponent和ProductQuotationInfo）
- 搜索和筛选API
- 评价系统API
- 推荐算法
- 产品数据导入脚本
- 产品图片生成脚本
- 国际化翻译键（shop.products.\*）

**实施说明**:

- 在v2.0.0中完成基础商城功能
- 在v2.1.0中完成产品详情页优化、配置清单、报价说明、数据导入和国际化修复
- 所有商城功能已实现并优化
...
- [x] 用户权限管理（USER, CREATOR, REVIEWER, FACTORY_MANAGER, ADMIN,
      SUPER_ADMIN）
- [x] 邮箱管理（修改邮箱、验证状态检查）
...
**描述**: 支持中文和英文的多语言支持，具有动态语言切换和本地化内容，所有页面和组件完全国际化。
...
- [x] 语言切换功能集成到Header
- [x] 多语言路由优化
- [x] 商品商城完整国际化（商品列表、商品详情、筛选组件、商品卡片）
- [x] 修复所有硬编码文本（替换为翻译键）
- [x] 商品详情页完整翻译（配置清单、报价说明、评价等）

**技术要求**:

- next-intl库集成
- 带有[locale]段的动态路由
- 所有语言的翻译文件（messages/zh-CN.json, messages/en-US.json）
- 语言环境检测的中间件（localePrefix: 'always'）
- 语言切换器组件
- 完整的翻译键结构（shop.products.*, shop.products.detail.*等）

**实施说明**:

- 在v2.0.0中完成基础国际化
- 在v2.1.0中完成商品商城国际化修复和所有硬编码文本替换
- 所有组件已翻译，无硬编码文本
- 语言切换正常工作
- 路由中间件已优化
...
**描述**: 完整的部署系统，支持蓝绿部署策略，实现零停机部署和快速回滚。

**验收标准**:

- [x] 目录-based蓝绿部署方案
- [x] 自动化部署脚本（deploy-blue-green-directory.sh）
- [x] 环境切换（blue/green环境切换）
- [x] 健康检查机制
- [x] 部署日志记录
- [x] 快速回滚功能
- [x] PM2进程管理集成
...
- 目录结构（blue/green环境目录）
- 部署脚本（rsync、ssh、pm2命令）
- 健康检查API
- 部署文档（BLUE_GREEN_DEPLOYMENT.md）
...
OpenAero 构建在现代、可扩展的架构上，使用Next.js 14+和App Router、TypeScript
5+以及面向微服务的设计。该平台利用云原生技术实现可靠性和可扩展性。
...
- [x] 分析和报告（监控仪表盘、性能监控）
- [x] 产品商城优化（详情页、配置清单、报价说明、数据导入）
- [x] 国际化完善（商品商城完整翻译、硬编码文本修复）
- [x] 蓝绿部署系统（零停机部署、快速回滚）
- [ ] 移动应用程序（计划中）
...
- ✅ 完成产品商城详情页优化（配置清单、报价说明、产品组件展示）
- ✅ 完成产品数据导入功能（从HTML文档批量导入植保无人机产品）
- ✅ 完成产品图片管理（SVG占位图生成、图片路径更新）
- ✅ 完成国际化翻译修复（商品商城完整翻译、所有硬编码文本替换）
- ✅ 完成蓝绿部署系统（目录-based部署、零停机部署、快速回滚）
- ✅ 完善商品详情页功能（部件分类、规格说明、报价信息、服务条款）
