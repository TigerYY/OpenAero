#!/bin/bash

# Git 历史清理脚本 - 使用 BFG Repo-Cleaner
# 更适合已经修改过的仓库

set -e

echo "═══════════════════════════════════════════════════════════════"
echo "🔥 Git 历史清理脚本 (BFG 方法)"
echo "═══════════════════════════════════════════════════════════════"
echo ""

# 颜色定义
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m' # No Color

echo -e "${YELLOW}⚠️  警告：此操作会重写 Git 历史！${NC}"
echo ""

# 需要从历史中删除的文件
FILES_TO_REMOVE=(
    "DATABASE_QUICK_REFERENCE.md"
    "DATABASE_CONNECTION_FIXED.md"
    "DATABASE_CONNECTION_FIX_GUIDE.md"
    "DATABASE_FIX_SUMMARY.md"
    "RESET_DATABASE_PASSWORD.md"
    "GET_CORRECT_PASSWORD.md"
    "FINAL_PASSWORD_RESET_STEPS.md"
    "SUPABASE_DIRECT_CONNECTION_STRING.md"
)

echo -e "${YELLOW}将要删除的文件:${NC}"
for file in "${FILES_TO_REMOVE[@]}"; do
    echo "  - $file"
done
echo ""

# 确认操作
read -p "$(echo -e ${RED}是否继续? 输入 'YES' 确认: ${NC})" confirm
if [ "$confirm" != "YES" ]; then
    echo -e "${YELLOW}操作已取消${NC}"
    exit 0
fi

echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "步骤 1: 检查依赖"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

# 检查 BFG 是否安装
if ! command -v bfg &> /dev/null; then
    echo -e "${RED}❌ BFG 未安装${NC}"
    echo ""
    echo "请使用以下命令安装："
    echo "  brew install bfg"
    echo ""
    exit 1
fi

echo -e "${GREEN}✓ BFG 已安装${NC}"
BFG_VERSION=$(bfg --version 2>&1 | head -1)
echo "  版本: $BFG_VERSION"
echo ""

echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "步骤 2: 备份当前仓库"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

BACKUP_DIR=".git.backup.$(date +%Y%m%d_%H%M%S)"
echo "📦 创建备份: $BACKUP_DIR"

cp -r .git "$BACKUP_DIR"
echo -e "${GREEN}✓ 备份完成${NC}"
echo ""

echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "步骤 3: 创建文件删除列表"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

DELETE_LIST="files-to-delete.txt"
rm -f "$DELETE_LIST"

for file in "${FILES_TO_REMOVE[@]}"; do
    echo "$file" >> "$DELETE_LIST"
done

echo -e "${GREEN}✓ 文件列表已创建: $DELETE_LIST${NC}"
cat "$DELETE_LIST"
echo ""

echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "步骤 4: 使用 BFG 清理历史"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

echo "🗑️  执行清理..."
bfg --delete-files "$DELETE_LIST" .

echo -e "${GREEN}✓ BFG 清理完成${NC}"
echo ""

echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "步骤 5: 清理和压缩仓库"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

echo "🧹 清理 reflog..."
git reflog expire --expire=now --all

echo "📦 压缩仓库..."
git gc --prune=now --aggressive

echo -e "${GREEN}✓ 清理和压缩完成${NC}"
echo ""

echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "步骤 6: 验证清理结果"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

echo "🔍 检查文件是否仍在历史中..."
FOUND_FILES=0

for file in "${FILES_TO_REMOVE[@]}"; do
    if git log --all --full-history --source -- "$file" 2>&1 | grep -q "commit"; then
        echo -e "${RED}❌ $file 仍在历史中${NC}"
        FOUND_FILES=1
    else
        echo -e "${GREEN}✓ $file 已从历史中删除${NC}"
    fi
done

echo ""

if [ $FOUND_FILES -eq 1 ]; then
    echo -e "${RED}⚠️  警告: 某些文件仍在历史中${NC}"
    echo "这可能是正常的，BFG 保护最新的提交"
    echo ""
fi

echo -e "${GREEN}✅ 历史清理完成！${NC}"
echo ""

# 清理临时文件
rm -f "$DELETE_LIST"

echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "步骤 7: 强制推送（必需）"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

echo ""
echo -e "${YELLOW}⚠️  现在需要强制推送到远程仓库以完成清理${NC}"
echo ""
echo "执行以下命令："
echo ""
echo -e "${BLUE}  git push --force origin 006-user-auth-system${NC}"
echo ""
echo "⚠️  警告："
echo "  - 这会覆盖远程分支的历史"
echo "  - 如果有团队成员，他们需要重新克隆仓库"
echo ""

read -p "$(echo -e ${RED}是否现在强制推送? (y/n) ${NC})" -n 1 -r
echo ""

if [[ $REPLY =~ ^[Yy]$ ]]; then
    echo "🚀 正在强制推送..."
    git push --force origin 006-user-auth-system
    echo -e "${GREEN}✅ 推送完成！${NC}"
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "✅ Git 历史清理完全完成！"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo ""
    echo "下一步："
    echo "  1. 在 GitHub 上验证敏感信息已删除"
    echo "     访问: https://github.com/TigerYY/OpenAero"
    echo "     搜索: 4gPPhKf90F6ayAka"
    echo "     预期: 0 个结果"
    echo ""
    echo "  2. 如有团队成员，通知他们重新克隆"
    echo ""
    echo "备份位置: $BACKUP_DIR"
    echo ""
else
    echo -e "${YELLOW}跳过推送${NC}"
    echo ""
    echo "要完成清理，请手动执行："
    echo "  git push --force origin 006-user-auth-system"
    echo ""
fi

echo -e "${GREEN}🎉 脚本执行完成！${NC}"
echo ""
echo "📝 BFG 报告位置: .git/bfg-report/$(date +%Y-%m-%d)/"
